                ...
                if accountCheck.count == 0 {
                    ...
                    if (addStatus != errSecSuccess) {
                        if let addErr = SecCopyErrorMessageString(addStatus, nil) {
                            WriteToLog().message(stringOfText: "[addStatus] Write failed for new credentials: \(addErr)")
                        }
                    }
                } else {
                    ...
                    if (updateStatus != errSecSuccess) {
                        if let updateErr = SecCopyErrorMessageString(updateStatus, nil) {
                            WriteToLog().message(stringOfText: "[updateStatus] Update failed for existing credentials: \(updateErr)")
                        }
                    }
                }
            }
